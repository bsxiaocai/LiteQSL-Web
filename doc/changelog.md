...
#### 1. 完善访问端表格显示
- **频率旁显示波段**：在频率后使用小字体显示对应波段，如 `14.270 MHz (20m)`
- **中继通联显示优化**：统一中继通联与卫星通联的展示方式
  - 填写中继名称时：第一行显示中继名称，第二行显示上下行频率
  - 未填写中继名称时：直接显示上下行频率
- **Eyeball QSO 地点显示**：鼠标悬停类型标签时显示地点信息（HTML title）
- **日期时间格式统一**：统一显示为 `YYYY-MM-DD HH:MM:SS` 格式

#### 2. 完善管理端手动录入 QSO 界面
- **Eyeball QSO 新增地点字段**：当选择 Eyeball QSO 时，额外显示"地点"字段
  - 非必填，支持中英文
  - 仅在 Eyeball QSO 类型下显示

#### 3. 统一 QSO 类型命名
- 统一使用 `👀 Eyeball QSO` 作为显示名称
- 包括访问端、管理端、标签显示、表格显示
...
| 文件 | 修改原因 |
|------|----------|
| `static/index.html` | 优化频率显示、日期时间格式、类型标签悬停提示 |
| `static/admin.html` | 同步访问端优化、新增 Eyeball 地点字段、统一命名 |
| `app/database.py` | 添加 qth 字段支持 |
| `doc/changelog.md` | 更新更新日志 |
