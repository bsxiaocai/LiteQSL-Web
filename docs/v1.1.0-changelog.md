...
LiteQSL-Web v1.1.0 是一个重要功能更新版本，主要针对 QSO 类型系统进行了全面重构，实现了类型驱动的表单动态显示，大幅提升了用户体验。同时优化了首次登录流程和日期时间输入方式。
...
**新增函数**：
- `setupAddFormTypeToggle()`：录入表单类型切换
- `setupEditFormTypeToggle()`：编辑弹窗类型切换
...
2. 重启服务：`python run.py` 或 `sudo systemctl restart liteqsl`
3. 无需数据库迁移，现有数据完全兼容
